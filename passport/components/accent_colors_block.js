/**
 * DiArt Passport
 * File: passport/components/accent_colors_block.js
 * Version: 1.0.0
 *
 * PAGE 2 — BLOCK: ACCENT COLORS.
 *
 * Rules:
 * - exactly 6 colors from palette.groups.accent;
 * - English name + Russian name + HEX;
 * - long names wrap to a second line, never ellipsis;
 * - visual language mirrors neutral_colors_block;
 * - no invented color names.
 */

"use strict";

const ACCENT_COLORS_BLOCK_VERSION = "1.0.0";
...
function collectAccentColors(palette) {
  const safePalette = obj(palette);
  const groups = obj(safePalette.groups);

  if (arr(groups.accent).length) {
    return arr(groups.accent).slice(0, 6);
  }

  return arr(safePalette.colors)
    .filter(color => color && color.group === "accent")
    .slice(0, 6);
}

function buildAccentColorsBlock({
  palette,
  x = 394,
  y = 500,
  width = 350,
  ...
  const colors = collectAccentColors(palette);
  const innerX = x + 16;
  const swatchY = y + 57;
  const gap = 8;
...
module.exports = {
  ACCENT_COLORS_BLOCK_VERSION,
  buildAccentColorsBlock
};
